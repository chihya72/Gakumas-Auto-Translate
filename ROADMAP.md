...
| B.12 | AI 机翻翻译记忆（TM） | ✅ | `tools/vendor/tm.ts`：加载 `csv_data/` 历史译文，按剧本/角色生成 REF 前文；仅 `pstory`/`pevent` 使用包含类型、行类型、说话人和完整原文的严格整行复用，人工优先、机翻回退，同级歧义安全跳过；同文件分批顺序翻译、前批译文自动成为后批前文 |
...
| B.16 | 策略矩阵改版（推翻 B.13） | ✅ | cidol/csprt 同组 01~03 **合并成一次请求**（管线侧拼接 + 按行数拆回，`tools/test_group_merge.py` 保往返一致）；event 顺序翻译 + 抽词生成本活动临时术语表；dear 改**滚动摘要 + 上一话全文**，并拒绝缺话摘要；pstory/pevent 使用严格整行复用；删掉 csprt 同批次兜底与参考行按长度排序 |
| B.19 | vendor 同步与 Dear 摘要重建 | ✅ | `vendor_sync.py` 按 SHA-256 同步本仓引擎文件；`build-dear-summaries.ts` 按人工层优先级选择来源、合并同话分段、复用未受影响检查点；自动管线写临时摘要，独立 workflow 维护正式摘要 |
...
### K. 个人ID绑定 ✅
> 工作台显示个人ID，不显示 GitHub ID。组员可自助修改自己的个人ID，管理员可维护全员字典。
...
| K.3 | 设置入口：组员自助修改自己的个人ID | ✅ 管理页顶部“我的个人ID” |
...
| L.4 | 管理员上传新文档 v1：网页上传 CSV → 推 `ai_csv/` 或指定阶段目录 + 自动开认领 issue | ✅ 单个 CSV 上传；按 AI/翻译/校对阶段创建对应状态 |
| L.5 | 批量选择+批量存档（依赖 I） | ✅ 管理页勾选后批量“过时存档” |
...
**当前实施顺序**：H.1 分页（前置缺陷）→ J（阶段目录与下载按钮）→ H（历史页）→ I（存档）→ K（个人ID/权限基础）→ L（权限与管理）。当前工作台二期主线完成；后续只做实际使用中发现的细化。
